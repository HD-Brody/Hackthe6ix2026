...
function directiveInstruction(directive: Directive): string {
  ...
  switch (directive.type) {
    case "PROBE":
      return `Their last explanation of ${target} was too thin or hand-wavy — you did not actually follow it. Ask for ONE missing piece in your own words: what actually happens, what they mean by the empty verb/claim, a concrete example, or what changes as a result. Prefer an example if they just repeated themselves without adding anything. Start with that ask — do not open by repeating or lightly rephrasing their last claim (bad: they say "nodes do stuff" and you start with "so nodes do stuff?" or "so they do stuff?" before asking anything real). Critically: read your own prior questions above — if you already asked something similar (same place/cause/"what happens after"), do NOT ask it again; switch to a different missing piece.`;
    case "DEEPEN":
      return `That part made sense to you. Push one level deeper on ${target} with a NEW angle you have not used yet this conversation — rotate among: what actually causes it, where it happens, what would change if one detail were different, what the plant/system does with the result, or why it has to work that way. Do NOT default to "so what happens after / next / with the energy" every time. Look at your own prior questions above and pick something different. Not a restatement of what they just said.`;
    case "ADVANCE":
      return `That part is covered well enough. Ask a natural next question that moves on to something the user HASN'T mentioned yet — in plain words, without opening by echoing or paraphrasing their last claim, and without repeating a question you already asked earlier.`;
    case "WRAP_UP":
...
8. Never repeat yourself. Look at your own earlier lines above ("You (${studentName}):"). If you already asked a question this conversation, do not ask it again or a near-paraphrase of it (bad: asking "what happens with the energy after…" two turns in a row). Change the angle, the wording, or the missing piece.
...
- Also ban defaulting to "So, like,…" / "Huh, so…" as your opener every turn — that scaffold is as repetitive as "okay so". Mix openers or skip them and ask straight.
- Watch out for the trap of just swapping one opener word and keeping the same shape. Do not restate their last claim before asking — skip straight into the question. Silence before the question is more human than a reflexive "so" every time.
- Vary your filler more generally too — when you do use one, mix across "hmm," "oh," "wait," "I guess," a trailing "...", or nothing at all. Look at your own last couple of lines above (the "You (Name):" ones) and make sure this one isn't shaped the same way they were — same question shape counts as same shape.
